	...
	Run: func(cmd *cobra.Command, args []string) {
		...
		for t := range torrentFiles {
			...
			for _, f := range filesToProcess {
				...
				case numberOfMatchingFiles > 1:
					// TODO : Add flag to skip this case to allow auto mode
			...
			// TODO : Add failure or success % and move or rename torrent according to the result
			matchedPercentage := float64(matchedSize) / float64(totalSize)
			switch {
			case matchedPercentage < minimalMatch:
				log.Println("\033[1;31m[-] Failed with\033[0m")
			case matchedPercentage >= minimalMatch:
				log.Println("\033[1;32m[-] Passed with\033[0m")
			}
		}
	},
}
